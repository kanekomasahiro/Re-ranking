...
def _sentence_to_ids(sentence, vocab):
  ...
  if FLAGS.case_sensitive:
    ids = [int(vocab.get(w, UNK_ID)) for w in sentence]
  else:
    ids = [int(vocab.get(w.lower(), UNK_ID)) for w in sentence]
  return ids


def _create_serialized_example(current, label, vocab):
  """Helper for creating a serialized Example proto."""
  example = tf.train.Example(features=tf.train.Features(feature={
      "features": _int64list_feature(_sentence_to_ids(current, vocab)),
...
def _process_input_file(filename, vocab, stats):
  ...
  for sentence_str in tf.gfile.FastGFile(filename):
    ...
    serialized = _create_serialized_example(sentence_tokens, label, vocab)
